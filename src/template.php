...
if ((in_array($target, $darkModeSites)) && isLoggedIn()) { ?>
    <div class="d-flex">
        <button class="btn btn-lg btn-primary" id="theme-switch"></button>
<?php }
if (isLoggedIn()) {
    if ($target === "edit_user") { ?>
        <a class="btn btn-lg btn-primary navbar-button" role="button" href="../logout?token=<?php echo $_SESSION['get_token']; ?>" id="navLoginLogout">Log out</a>
    <?php } else { ?>
        <a class=" btn btn-lg btn-primary navbar-button" role="button" href="logout?token=<?php echo $_SESSION['get_token']; ?>" id="navLoginLogout">Log out</a>
    <?php }
} else {
    if ($target === "edit_user") { ?>
        <a class="btn btn-lg btn-primary navbar-button" role="button" href="../login" id="navLoginLogout">Log in</a>
    <?php } else { ?>
        <a class="btn btn-lg btn-primary navbar-button" role="button" href="login" id="navLoginLogout">Log in</a>
    <?php } 
} 
